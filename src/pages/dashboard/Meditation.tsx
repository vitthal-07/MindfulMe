  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-12">
        <h1 className="text-4xl font-bold text-slate-800 mb-4">Meditation Practice</h1>
        <p className="text-xl text-slate-600">Find peace through guided meditation and mindfulness</p>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {techniques.map((technique, index) => (
          <div key={index} className="bg-white rounded-xl shadow-lg p-6">
            <h3 className="text-xl font-semibold text-slate-800 mb-3">{technique.title}</h3>
            <p className="text-slate-600 mb-4">{technique.description}</p>
            <p className="text-sm text-indigo-600 font-medium">{technique.duration}</p>
          </div>
        ))}
      </div>
    </div>
